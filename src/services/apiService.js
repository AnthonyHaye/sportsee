...
import { mockedUserData, mockedUserActivity, mockedUserAverageSessions, mockedUserPerformance } from './mockData'; // Importe les données mockées
...
export const fetchUserAverageSessions = async (userId) => {
  if (CONFIG.useMockData) {
      return mockedUserAverageSessions.find(sessions => sessions.userId === userId);
  } else {
      try {
...
export const fetchUserPerformance = async (userId) => {
  if (CONFIG.useMockData) {
      return mockedUserPerformance.find(performance => performance.userId === userId);
  } else {
      try {
